...
router.get('/', requireAuth, (req, res) => {
  ...
  if (user.role === 'student') {
    ...
    const reviews = db
      .prepare('SELECT * FROM sca_student_reviews WHERE student_id = ?')
      .all(user.id);
    const reviewMap = {};
    reviews.forEach((r) => {
      reviewMap[r.finding_id] = r;
    });
  ...
  const vmFindings = db.prepare("SELECT * FROM vulnerabilities WHERE source = 'sca'").all();
...
router.get('/stats', requireAuth, requireRole(['admin', 'professor']), (req, res) => {
  ...
  if (totalStudents > 0 && totalFindings > 0) {
    const totalPct = Object.values(submittedCounts).reduce(
      (sum, cnt) => sum + cnt / totalFindings,
      0
    );
  ...
  const pace = allReviews.filter((r) => r.submitted_at && r.submitted_at >= fiveMinAgo).length;
...
router.get('/findings/:id', requireAuth, (req, res) => {
  const finding = db
    .prepare('SELECT * FROM sca_findings WHERE id = ?')
    .get(parseInt(req.params.id));
  if (!finding)
    return res
      .status(404)
      .render('error', { message: 'Finding not found', error: { status: 404 } });
  ...
  if (user.role === 'student') {
    ...
    myReview = db
      .prepare('SELECT * FROM sca_student_reviews WHERE finding_id = ? AND student_id = ?')
      .get(finding.id, user.id);
  } else {
    allReviews = db
      .prepare('SELECT * FROM sca_student_reviews WHERE finding_id = ?')
      .all(finding.id);
  }
  ...
  const users =
    user.role !== 'student'
      ? db.prepare('SELECT id, username FROM users WHERE role = ?').all('student')
      : null;
...
router.get('/student/:studentId', requireAuth, requireRole(['admin', 'professor']), (req, res) => {
  ...
  if (!student)
    return res
      .status(404)
      .render('error', { message: 'Student not found', error: { status: 404 } });
